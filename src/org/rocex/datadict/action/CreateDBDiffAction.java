...
public class CreateDBDiffAction implements IAction
{
    ...
    {
        this.strVersion1 = Context.getInstance().getSetting("diff.version1");
        this.strVersion2 = Context.getInstance().getSetting("diff.version2");
    ...
    private void createNewTableSQLFile(String strNewTableColumn, SQLParameter sqlParam, String strFileName)
    {
        ...
        for (Map.Entry<String, List<DiffColumn>> entry : mapTable.entrySet())
        {
            ...
            String strColumns = entry.getValue().stream().map(column ->
            {
                return "\t%s %s comment '%s'".formatted(column.getColumnName().toLowerCase(), column.getMerge2(), column.getColumnShowName());
    ...
    public void doAction(EventObject evt)
    {
        ...
        List<DiffColumn> listDiffColumn2 = createDiff(DiffColumn.class, strColumnSQL, new SQLParameter(strVersion2, strVersion1, strVersion2, strVersion1),
            new DiffToStringColumn4(), "07 " + strFileName + "删除字段.txt");
