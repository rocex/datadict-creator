 ...
 ***************************************************************************/
public class GenerateJsonDataDictAction extends GenerateDataDictAction
{
    /***************************************************************************
     * @author Rocex Wang
     * @version 2020-4-26 14:52:18
     ***************************************************************************/
    public GenerateJsonDataDictAction(String strVersion)
    {
        super(strVersion);
    ...
    protected void createDataDictFile(ClassVO classVO, List<PropertyVO> listPropertyVO)
    {
        ...
        String strHtmlDataDictRow = DataDictGenerator.settings.getProperty("HtmlDataDictRow");
        ...
        String strHtml = MessageFormat.format(DataDictGenerator.settings.getProperty("HtmlDataDictFile"), classVO.getDisplayName(),
                classVO.getDefaultTableName(), strFullClassname, DataDictGenerator.settings.get(strVersion + ".DataDictVersion"), strClassList, strHtmlRows,
                getFooter());
        ...
        JacksonJsonFilter filters = new JacksonJsonFilter();

        filters.filter(ClassVO.class, "classType", "componentId", "keyAttribute", "id");
        filters.filter(PropertyVO.class, "id", "classId", "dataType", "dataTypeStyle");

        mapper.setFilterProvider(filters);

        mapper.addMixIn(ClassVO.class, filters.getClass());
        mapper.addMixIn(PropertyVO.class, filters.getClass());

        try
        {
            File objFile = getClassFilePath(classVO).toFile();
            objFile.createNewFile();

            mapper.writerWithDefaultPrettyPrinter().writeValue(objFile, classVO);
        }
        catch (IOException ex)
        {
            Logger.getLogger().error(ex.getMessage(), ex);
    ...
    protected void createIndexFile()
    {
        ...
        String strHtml = MessageFormat.format(DataDictGenerator.settings.getProperty("HtmlIndexFile"),
                DataDictGenerator.settings.get(strVersion + ".DataDictVersion"));
    ...
    {
        return Paths.get(strOutputDdcDir, getMappedClassId(classVO) + ".json");
    }
    ...
    {
        String strFooter = DataDictGenerator.settings.getProperty("HtmlDataDictFooterFile");
...
class JacksonJsonFilter extends FilterProvider
{
